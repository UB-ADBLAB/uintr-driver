...
#define UINTR_PROC_HASH_BITS 6 // 2^6 = 64 buckets (64 spaces in uitt)
...
static void uintr_trace_sched_migrate_task(void *data, struct task_struct *p,
  ...
  if (proc->upid->nc.ndst != new_ndst) {
    ...
    // dump the updated UPID state for debug
    uintr_dump_upid_state(proc->upid, "sched_migrate");
  }
}

// Tracepoint probe registration function
static int register_sched_tracepoints(void) {
  int ret;

  ret = register_trace_sched_migrate_task(uintr_trace_sched_migrate_task, NULL);
  if (ret) {
...
static void unregister_sched_tracepoints(void) {
  unregister_trace_sched_migrate_task(uintr_trace_sched_migrate_task, NULL);
}
...
int uintr_sched_trace_register_proc(struct uintr_process_ctx *proc) {
  ...
  // set NDST to current CPU's APIC ID
  if (proc->upid) {
